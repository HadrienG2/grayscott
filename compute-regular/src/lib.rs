...
fn step_center(species: &mut Species, params: &Parameters) {
    ...
    {
        // Compute pixel
        let (new_u, new_v) = compute_pixel(win_u, win_v, STENCIL_SHAPE, stencil_offset, params);
        *out_u = new_u;
        *out_v = new_v;
    }
}
...
fn compute_pixel(
    ...
    let dv =
        params.diffusion_rate_v * full_v + uv_square - (params.feed_rate + params.kill_rate) * v;
